  ...
  async generateLayout() {
    ...
    try {
      ...
      if (result.success && result.data.image_base64) {
        // 跳转到预览页面
        wx.redirectTo({
          url: `/pages/preview/preview?layoutData=${encodeURIComponent(JSON.stringify(result.data))}&singleData=${encodeURIComponent(JSON.stringify({ image_base64: this.data.currentImageUrl }))}&name=${this.data.photoType.name}`
        });
      } else {
        wx.showToast({
          title: '生成失败，请重试',
